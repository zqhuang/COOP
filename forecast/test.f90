...
#include "constants.h"
  ...
  call fig%init(xlabel = "$\ell$", ylabel = "$C_\ell / C_\ell^{\rm \Lambda CDM} - 1$", xlog=.true., ymin = -0.06, ymax = 0.06)
  ...
  call fig%interpolate_curve(ells, Cl(1,:)/lcdm_Cl(1,:) - 1.d0,  "LogLinear", color="red", linetype="dashed", linewidth=2.5, legend="$\alpha_{M0} = 0.2$, unlensed TT")
  call fig%interpolate_curve(ells, Cl(4,:)/lcdm_Cl(4,:) - 1.d0, "LogLinear", color="blue", linetype="dashed", linewidth=1., legend="$\alpha_{M0} = 0.2$, $\phi\phi$")  
