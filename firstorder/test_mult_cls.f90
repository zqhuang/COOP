...
!!$  call fp%close()
  ...
  call fig%init(xlabel = "$\ell$", ylabel = "$\frac{\ell(\ell+1)C_{\ell}}{2\pi} (\mu K^2)$ ", xlog = .true., xmin = 1.8, xmax = 2200., caption="Coupled Dark Energy $Q = \frac{\partial \ln m_{DM}}{\partial \phi}$")
  do l=lmin, lmax
     ells(l) = l
  enddo
...
!!$  call fig%curve(ells, Cls_scalar(coop_index_ClTT,:)*ells*(ells+1.d0)*(norm/coop_2pi), linewidth=1., legend="$\Lambda$CDM")
  ...
  call fig%label("$V=V_0\phi^{-0.3}$", 0.1, 0.4, alignment = "left")
