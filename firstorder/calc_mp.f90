...
#include "constants.h"
  ...
  type(coop_list_double)::redshifts
  ...
  call coop_dictionary_lookup(params, "output_redshift", redshifts)
  if(redshifts%n .le. 0)call redshifts%push( 0.d0 )
  do iz=1, redshifts%n
     write(*,"(A, F12.3)") "Computing power spectrum at redshift z = ", redshifts%element(iz)
  enddo
  ...
  do iz=1, redshifts%n
     ...
     call cosmology%get_Matter_power(z=redshifts%element(iz), nk = nk, k = k, Pk = matterPk)
     !!compute k^3 [ k^2/a^2 Phi_k /(3/2H^2\Omega_m) ]^2/(2pi^2)
     call cosmology%get_Phi_power(z=redshifts%element(iz), nk = nk, k = k, Pk = PhiPk)
     !!compute k^3 [ k^2/a^2 Psi_k /(3/2H^2\Omega_m) ]^2/(2pi^2)  
     call cosmology%get_Psi_power(z=redshifts%element(iz), nk = nk, k = k, Pk = PsiPk)
     ...
     write(*,*) "Saving the matter power spectrum in "//trim(output)//"_matterpower_"//COOP_STR_OF(redshifts%element(iz))//".dat"
     call fp%open(trim(output)//"_matterpower_z"//COOP_STR_OF(redshifts%element(iz))//".dat","w")
