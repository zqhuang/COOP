...
#include "constants.h"
  ...
  type, extends(coop_file) :: coop_asy
   ...
   contains
     ...
     procedure::plot => coop_asy_curve_mindis
...
100 write(this%unit, "(A)") "CURVE"
  ...
  subroutine coop_asy_curve_mindis(this, x, y, color, linetype, linewidth, legend)
    class(coop_asy) this
    COOP_REAL,parameter::min_dsq = 2.5d-5 !!about what your eye resolution
    COOP_REAL ,dimension(:),intent(IN)::x,y
    COOP_SINGLE,dimension(:),allocatable::xuse, yuse
    COOP_REAL:: xscale, yscale
    COOP_UNKNOWN_STRING,optional:: color, linetype
    COOP_SINGLE ,optional::linewidth
    COOP_UNKNOWN_STRING, optional::legend
    COOP_INT  i,n, nuse
    COOP_STRING lineproperty
    n = coop_getdim("coop_asy_curve", size(x), size(y))
    if(n .lt. 2) stop "only one point for coop_asy_curve_mindis?"
    yscale = max(maxval(y) - minval(y), 1.d-20)
    xscale = max(maxval(x) - minval(x), 1.d-20)
    allocate(xuse(n), yuse(n))
    nuse = 1
    xuse(1) = x(1)
    yuse(1) = y(1)
    do i=2, n-1
       if( ((x(i) - xuse(nuse))/xscale)**2 + ((y(i)-yuse(nuse))/yscale)**2 .gt. min_dsq)then
          nuse = nuse+1
          xuse(nuse) = x(i)
          yuse(nuse) = y(i)
       endif
    ...
    write(this%unit, "(A)") "NULL" !!no marker
    do i=1,nuse
       call this%write_coor(xuse(i), yuse(i))
    enddo
  end subroutine coop_asy_curve_mindis
...
100 continue
  ...
  subroutine coop_asy_histogram(x, nbins, filename, xlabel, ylabel, fit_gaussian)
    ...
    if(present(xlabel))then
       if(present(ylabel))then
          call fig%init(xlabel = xlabel, ylabel = ylabel)
       else
          call fig%init(xlabel = xlabel, ylabel = "P")
       endif
    else
       if(present(ylabel))then
          call fig%init(xlabel = "x", ylabel = ylabel)
       else
