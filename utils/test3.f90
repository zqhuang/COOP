!!cosmology with thawing k-essence dark energy and massive neutrino
!!by Zhiqi Huang, huangzhq25@mail.sysu.edu.cn 
!!use c/H_0 as the length unit, 1/H_0 the time unit, and \sqrt{3}M_p as the mass unit, rho_c = 3 M_p^2 H_0^2 as the density unit
...
!  use coop_wrapper_utils
  ...
  real*8, parameter:: fit_amin = 0.667d0 !!z=0.5
  !cold dark matter + baryon; this does not include massive neutrinos
  real*8, parameter::Omega_m = 0.31
  real*8, parameter:: min_cs2 = 0.d0, max_w = -0.333, min_w = -10., max_growth = 100.d0, frozen_cut  = 0.01d0
  real*8,parameter::rho0_min_ini = 0.1d0, rho0_max_ini = 10.d0
  ...
  real*8::Omega_L
  integer::index_start, n_used, ii_start, nn_used
...
contains
  ...
  function field_eq(y, lna, yp) result(fail) !y = [\phi, \dot \phi, H]
    ...
    if(check_physical)then
       ...
       if(p_DE >= max_w * rho_DE .or. p_DE < min_w * rho_DE)then  !truncate equation of state
  ...
  subroutine initialize()
    ...
    enddo
    a_list = exp(lna_list)
    n_used = nint(-log(fit_amin)/dlna)+1  !!fit ond-efold
  ...
  function solve_V00_scan(a, rho0_min, rho0_max, nscan) result(fail)
    ...
    integer nscan
    real*8 a
    real*8 y(NUM_VARS),  rho0_min, rho0_max, dlnrho 
    ...
    H_try = 0.d0
    best_index = 0
    dlnrho = log(rho0_max/rho0_min)/(nscan-1)
    do i=1, nscan
       rho0_try(i) = rho0_min*exp(dlnrho*(i-1))
    enddo
    rho0_try(0) = rho0_min
...
#include "constants.h"  
  integer i
  integer,parameter::want_samples = 1000
  real*8, parameter::w0_left = -1.22, w0_right = -1.d0+0.22
  real*8 w0(want_samples), wa(want_samples)
  type(coop_asy)::w0wa
  integer ncount
  call initialize()
  call w0wa%open("wfigs/wwscatter_"//COOP_STR_OF(nint(Omega_m*100))//"_"//COOP_STR_OF(nint(rand_width))//"_"//COOP_STR_OF(dimX)//"_"//COOP_STR_OF(dimV)//"_"//COOP_STR_OF(log10(rho0_min_ini))//"_"//COOP_STR_OF(log10(rho0_max_ini))//".txt")
  call w0wa%init(xlabel="$1+w_0$", ylabel="$w_a$", xmin = real(1.+w0_left), xmax = real(1.+w0_right), ymin = -0.4, ymax = 0.4, doclip = .true.)
  ncount = 0
  ...
  enddo
  call w0wa%plot( x = (/ 1.d0+w0_left, 1.d0+w0_right /), y = (/ kessence_wa(w0_left, Omega_m), kessence_wa(w0_right, Omega_m) /), color="orange", linetype="solid", linewidth=2., legend="$\Omega_m=0."//COOP_STR_OF(nint(Omega_m*100))//"$")   
  call w0wa%dots(1.d0+w0, wa, "skyblue")
  call w0wa%close()
end program main
