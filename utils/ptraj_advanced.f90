...
!! gfortran -O3 ptraj_advanced -o calcp
!! and run it with
!! ./calcp 
!! output format: 7 columns
!! t, x, y, z, vx, vy vz (units: microsecond, mm, mm, mm, m/s, m/s, m/s)
module pt_subs
  implicit none
  ...
  !!---------------------USER SECTION -----------------------------------
  !!you are only supposed to modify variables in this section
  !!----------------------------------------------------------------------
  !!input and output files
  character(LEN=*),parameter::input_file="19_05_18_01.csv"  !!input csv file
  character(LEN=*),parameter::output_file="ptraj2.txt"  !!output file  
  !!physical parameters
  real(kind=DL),parameter::mass = 171.0_dl * 1.6605e-27_dl !!mass in SI unit
  real(kind=DL),parameter::charge = 1.6e-19_dl !!charge in SI unit
  real(kind=DL),parameter::norm = 43503.0_dl !!potential/E normalization
  real(kind=DL),dimension(3),parameter::xini = (/ 0._dl,  0._dl, 2.248_dl /) * 1.e-3_dl !!initial position in SI unit
  real(kind=DL),dimension(3),parameter::vini = (/ 1.0_dl, 1.0_dl, 1.0_dl /)  !!initial velocity in SI unit
  real(kind=DL),parameter::freq = 2.e7_dl  !!sin modulation frequency
  real(kind=DL),parameter::tend = 10._dl * 1.e-6_dl  !!end time in SI unit
  !!methods
  integer(kind=IB),parameter::interpolation_method = 1
  !!    0: cubic in all directions; using potential U (often inaccurate due to discontinuity of E)
  !!    1: linear in all directions; using E field;
  !!    2: cubic in z direction but linear in x, y directions; using E field;
  !!    3: cubic in all directions; using E field.
  integer(kind=IB),parameter::ode_method = 0
  !!    0:  RK4  (good enough in most cases)
  !!    1:  RK6
  !!    2:  RK8
  !!---------------------END OF USER SECTION-----------------------------  

  !!this file will be used to save unformatted data;
  character(LEN=*),parameter::temp_file="temp_save.dat"
  real(kind=DL),parameter::charge_by_mass = charge/mass  
  real(kind=DL),parameter::omega = 2.0_dl*pi*freq
  real(kind=DL),dimension(:,:,:,:),allocatable::E_field
  real(kind=DL),dimension(:,:,:),allocatable::potential
  integer(kind=IB)::nx(3)   
  real(kind=DL),dimension(3)::xmin,xmax, dx

  
contains

  !!automatically figure out dx, dy, dz and x_min, y_min, z_min, x_max, y_max, z_max
  subroutine get_configuration(filename)
    character(LEN=*)::filename
    integer(kind=IB)::i,j,k
    character(LEN=1024)::str
    integer(kind=IB)::ntot
    real(kind=DL)::line(7), x1, x2
    nx = 0
    open(10, FILE=filename)
    open(11, FILE=temp_file, form="unformatted")
    str = ""
    do while(str(1:9) .ne. "Spacings:")
       read(10,"(A)") str
    enddo
    read(10, *, END=20, ERR=20) line(1:3)
    dx = line(1:3)
    ...
    read(10, *, END=20, ERR=20) line
    write(11) line(4:7)
    xmin = line(1:3)

    x2 = xmin(2) + dx(2)/2._dl    
    do while( line(2) .lt. x2 )
       read(10, *, END=20, ERR=20) line       
       write(11) line(4:7)
       nx(3) = nx(3) + 1
    enddo
    ...
    nx(2) = 1
    x1 = xmin(1) + dx(1)/2._dl
    do while(line(1) .lt. x1)
       do i=1, nx(3)
          read(10, *, END=20, ERR=20) line       
          write(11) line(4:7)
       enddo
...
10  close(10)
    ...
    xmax = xmin + dx * (nx-1)
    return
20  write(*,"(A)") "Error in the data file: "//trim(filename)
    stop
  ...
  subroutine load_data(filename)
    ...
    real(kind=DL) line(7)
    write(*,*) "loading data..."    
    call get_configuration(filename)
    write(*,"(A, 3G14.5)") "Spacing: ", dx    
    write(*, "(A, 3G14.5)") "x_min, y_min, z_min:", xmin
    write(*, "(A, 3G14.5)") "x_max, y_max, z_max:", xmax
    if(allocated(E_field))deallocate(E_field)
    if(allocated(potential))deallocate(potential)
    allocate(E_field(3, nx(1), nx(2), nx(3)), potential(nx(1), nx(2), nx(3)))
    open(10, FILE=temp_file, form="unformatted")
    do i=1, nx(1)
       do j=1, nx(2)
          do k=1, nx(3)
             read(10, ERR=100, END=100) E_field(:, i, j, k), potential(i, j, k)
          enddo
       enddo
    enddo
    close(10)
    write(*,*) "data loaded"
    !!convert to SI units and multiply E, U with q/m
    xmin = xmin*1.e-3_dl
    dx = dx * 1.e-3_dl
    xmax = xmax*1.e-3_dl
...
100 stop "Error in the data file"
  ...
  subroutine eom(n, t, y, yp)
    ...
    case(2)
       call get_acceleration_with_E_interpz3(y(1:3), yp(4:6))
    case(3)
       call get_acceleration_with_E_interpz3(y(1:3), yp(4:6))
    case default
       stop "Unknown interpolation method. Please choose 0, 1, 2, 3"
    end select
...
program pt
  ...
  real(kind=DL)::y(n), t, yp(n), E(3)
  integer(kind=IB)::i
  real(kind=DL),parameter::stepsize = min(1.e-3_dl/freq, 1.e-9_dl)
  call load_data(input_file)
  !call load_data_quick()  
  y(1:3) = xini
  y(4:6) = vini
  t = 0.0_dl
  open(12, file=output_file)
  !!output unit: microsecond, mm, m/s !!test  
  write(12, "(7G14.5)") t*1.e6, y(1:3)*1.e3, y(4:6)
  select case(ode_method)
  case(0)
     do i=1, nint(tend/stepsize)
        call RungeKutta4th(n, eom, t, y, stepsize)
        if(mod(i, 100).eq.0)   write(12, "(7G14.5)") t*1.e6_dl, y(1:3)*1.e3_dl, y(4:6) 
     enddo
  case(1)
     do i=1, nint(tend/stepsize)
        call RungeKutta6th(n, eom, t, y, stepsize)
        if(mod(i, 100).eq.0)   write(12, "(7G14.5)") t*1.e6_dl, y(1:3)*1.e3_dl, y(4:6) 
     enddo
  case(2)
     do i=1, nint(tend/stepsize)
        call RungeKutta8th(n, eom, t, y, stepsize)
        if(mod(i, 100).eq.0)   write(12, "(7G14.5)") t*1.e6_dl, y(1:3)*1.e3_dl, y(4:6) 
     enddo
  case default
     stop "Unknown ODE method: please choose 0, 1, 2."
  end select
  close(12)
  deallocate(E_field, potential)
end program pt
