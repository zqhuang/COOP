...
contains
  ...
  subroutine QuN_calc_canonical_efolds(Hend, efolds, f_ini, dot_f_ini)
    real(kind=DL),parameter::accuracy = 3.e-4_dl
    real(kind=DL),parameter::epsilon_bias = 50.0_dl        
    integer(kind=IB),parameter::n = 2*User_nflds+2        
    ...
    do while(y(n) .gt. Hend)
       call QuN_canonical_RK4th(y, h, workspace)
       call check_step()
    enddo
  ...
  contains
    ...
    subroutine check_step()
      ...
      if(mod(count, update_freq) .eq. 0)then
         ...
         if(yp(n) .lt. 0.0_dl)then
            call update_step( max(min( accuracy / y(n) / (1.0_dl + epsilon_bias*eps) , &
                 (Hend-y(n))/yp(n)/(update_freq*1.1_dl) ), 1.e-13_dl/y(n)) )
         else
            call update_step( accuracy / y(n) / (1.0_dl + epsilon_bias*eps) )
         endif
      endif
...
program QuN_main
  ...
  real(kind=DL),dimension(User_nflds),parameter::f_ini = (/ 24.0_dl * Mpl, 1.e-5_dl*Mpl /)
  ...
  real(kind=DL),dimension(-n:n, -n:n)::efolds  !!save the results 
  !!----------------------------------------------------
  real(kind=DL)::H_ini, H_end, step
  integer(kind=IB)::i, j
  if(verbose>1) write(*,"(A, I3, A)") "Working with ", DL*8, "bits precision."
  H_ini = QuN_Canonical_Hubble( f_ini )
  H_end = QuN_Canonical_Hubble( f_ini/10.0_dl ) !!where to stop
