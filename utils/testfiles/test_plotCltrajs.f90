...
#include "constants.h"
  type(coop_asy)::fig
  character(LEN=*),parameter::genre = "TE"
  character(LEN=*),parameter::interp = "loglinear"  
  character(LEN=*),parameter::root = "cls/dpp11_"//genre//"_BK15_BAO"  
  ...
  if(interp .eq. 'loglinear')then
     call fig%init(width=8., height = 8., xlabel="$\ell$", ylabel="$D_l^{"//genre//"}$", xlog=.true., xmin=1.8, xmax=2600.)
  else
     call fig%init(width=8., height = 8., xlabel="$\ell$", ylabel="$D_l^{"//genre//"}$", xlog=.false., xmin=1.8, xmax=2520.)     
  endif
  i=1
  call plot_from_file(root//"_"//COOP_STR_OF(i)//".theory_cl", genre, "skyblue", "dotted", 0.8, "12-knots $1\sigma$ samples")  
  do i=2, 50
     call plot_from_file(root//"_"//COOP_STR_OF(i)//".theory_cl", genre, "skyblue", "dotted", 0.8)
  enddo
  call plot_from_file(root//"_best.theory_cl", genre, "blue", "solid", 1.5, "12-knots best")
  call plot_lcdm(genre, "red", "solid")  
  call plot_cl_data(genre, "darkgray")
  if(genre=="TT")then
     call fig%legend(xratio = 0.05, yratio = 0.92)
     call fig%label(xratio = 0.05, yratio = 0.6, label="12-knots data: "//genre//"+BK15+lensing+BAO")
  else
     call fig%legend(xratio = 0.05, yratio = 0.3)
     call fig%label(xratio = 0.05, yratio = 0.1, label="12-knots data: "//genre//"+BK15+lensing+BAO")
...
100 call fp%close()
  ...
  subroutine plot_lcdm(genre, color, linetype)
    ...
    call fig%interpolate_curve(xraw=lcdm_ells, yraw=lcdm_power(:, ind), interpolate=interp, color=color, linetype=linetype, linewidth=1., legend="$\Lambda$CDM best")
