...
#include "constants.h"      
  ...
  !!--------input parameters ---------------------------------
  !  COOP_REAL,parameter::z1 = 0.1d0, z2 = 5.d0, z3 = 29.195d0, f = 2.1d0, V1=0.d0, V2=1.d0, V3 = 0.d0
  COOP_REAL,parameter::z1 = 5.d0, z2 = 15.d0, z3 = 29.195d0, f = 2.1d0, V1=0.d0, V2=-1.d0, V3 = 0.d0   
  COOP_UNKNOWN_STRING,parameter::output_file = "potential.txt"
  !!----------------------------------------------------------
  COOP_REAL,parameter::c=1.d0/(4.d0*f)
  COOP_REAL,parameter::rmax=sqrt(z3/c), rcut12 = sqrt(z1/c), rcut23 = sqrt(z2/c), eps = 1.d-3   !!z = c r^2;
  ...
  COOP_INT,parameter:: is1 = ceiling(n * (rcut12/rmax)*1.1), is2=  ceiling(n * ((rcut23-rcut12)/rmax)*1.1), is3 = ceiling(n * ((rmax-rcut23)/rmax)*1.1)
  COOP_INT,parameter::n_samples = is1 + is2 + is3
  COOP_REAL::params(n_params)  
  COOP_REAL::V_samples(n_samples)
  COOP_REAL::p2V(n_samples, n_params)
...
contains
  ...
  subroutine set_up_rs()
    COOP_REAL::totl, dl, l1, l2, l3, dl1, dl2, dl3, delta_l
    COOP_INT::i
    l1 = coop_integrate(dlbydr, 0.d0, rcut12, 1.d-7)
    l2 = coop_integrate(dlbydr, rcut12, rcut23, 1.d-7)
    l3 = coop_integrate(dlbydr, rcut23, rmax, 1.d-7)
    totl = l1 + l2 + l3
    ...
    n3 = nint(l3/dl)+nbuff
    n2 = n - n1 - n3
    dl1 = (l1 - delta_l*(nbuff+1))/(n1-nbuff)
    dl2 = (l2 - delta_l*2*(nbuff+1))/(n2-2*nbuff)
    dl3 = (l3 - delta_l*(nbuff+1))/(n3-nbuff)    
    ...
    enddo
    dr(n1) = delta_l/dlbydr(rcut12)
    dr(n1+1) = dr(n1)
    r(n1) = rcut12 - 1.5d0*dr(n1)
    r(n1+1) = rcut12 + 1.5d0*dr(n1)
    do i=1, nbuff-1    
       dr(n1-i) = dr(n1)
       dr(n1+i+1) = dr(n1)
       r(n1-i) = rcut12 - (i+1.5)*dr(n1)
       r(n1+i+1) = rcut12 + (i+1.5)*dr(n1)       
    enddo
    ...
    do i = n1+nbuff+1, n1+n2-nbuff
       ...
       r(i) = r(i-1) + (dr(i-1)+dr(i))/2.d0 
    enddo
    dr(n1+n2) = delta_l/dlbydr(rcut23)
    dr(n1+n2+1) = dr(n1+n2)
    r(n1+n2) = rcut23 - 1.5d0*dr(n1+n2)
    r(n1+n2+1) = rcut23 + 1.5d0*dr(n1+n2)
    do i=1, nbuff-1    
       dr(n1+n2-i) = dr(n1+n2)
       dr(n1+n2+i+1) = dr(n1+n2)
       r(n1+n2-i) = rcut23 - (i+1.5)*dr(n1+n2)
       r(n1+n2+i+1) = rcut23 + (i+1.5)*dr(n1+n2)       
    enddo
...
end module parabolaU

program Test3
  use coop_wrapper_utils
  use parabolaU
  implicit none
  COOP_INT::i, nloop, nloop_total
  ...
  write(2, "(5A16)") "# z ", " surface sigma ", " surface V ", " on-axis V ", "10% off axis V" 
  do i=1, n
     if(i.ne. n1 .and. i.ne. n1+1 .and. i.ne. n1+n2 .and. i.ne. n1+n2+1) &
          write(2,"(5E16.7)") c*r(i)**2, sigma(i),  pot_at_a(r(i), n, r, dr, sigma, dsdr) , axis_potential(c*r(i)**2), full_potential(c*r(i)**2, r(i)*0.1d0)
  enddo
  close(2)
...
contains
  ...
  subroutine get_sigma()
    COOP_INT:: j
    COOP_REAL::pivot2 = (rcut12+rcut23)/2.d0, pivot3 = rmax
    a1 = params(1:pow1+1)
