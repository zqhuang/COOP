...
#include "constants.h"
  ...
  public::coop_write_matrix, coop_print_matrix, coop_read_matrix, coop_set_identity_matrix, coop_identity_matrix, coop_diagonal_matrix, coop_matrix_add_diagonal, coop_matrix_sum_columns, coop_matrix_Solve, Coop_matrix_Inverse, coop_matsym_xCx, coop_matrix_det_small,coop_matsym_mat2vec, coop_matsym_vec2mat, Coop_matsym_Diagonalize, coop_matsymdiag_small,  Coop_matsym_Sqrt,  coop_matsym_sqrt_small,  coop_matsym_power_small,  Coop_matsym_power,  coop_matsym_function, Coop_matsym_LnDet,  coop_matsym_index, coop_matrix_sorted_svd,  coop_covmat, coop_matrix_dominant_eigen_value
...
100 if(present(success))then
  ...
  subroutine coop_matrix_solve(m, n, neqs, a, b, nosave)
    COOP_INT m, n
    COOP_REAL a(m, n), b(n, neqs), acopy(n,n)
    COOP_INT indx(n), info
    logical::optional::nosave
#ifndef HAS_LAPACK
    COOP_REAL  d
#endif
    select case(n)
    case(1)
       b = b/a(1,1)
    case default
       if(present(nosave))then
          if(nosave)then
#ifdef HAS_LAPACK
             call dgesv(n, neqs, a, m, indx, b, n,info)
#else
             call coop_matrix_LUdcmp(a, indx, d, n)
             
             call coop_matrix_LUbksb(a, indx, b, n)
#endif
          else
             goto 100
          endif
...
100    acopy = a
  ...
  !!return Inverse of A
  Subroutine Coop_matrix_Inverse(m, n, A)
    COOP_INT::m, n
    COOP_REAL::A(m, n) !!m>=n
    COOP_INT info
#ifdef HAS_LAPACK
  ...
  subroutine coop_sympos_solve(n, A, b, nosave)
    COOP_INT n, info
    COOP_REAL  a(n,n), acopy(n,n)
    COOP_REAL::b(n)
    logical,optional::nosave
    select case(n)
    case(1)
    ...
    case default
       if(present(nosave))then
          if(nosave)then
             ...
             if(info .gt. 0) stop "sympos_solve: matrix is not positive definit"
             if(info .lt. 0) stop "sympos_solve: unknown error"
#else
             call Coop_cholesky(n, n, a, info)
             if(info .ne. 0) stop "sympos_solve: matrix is not positive definit"
             call Coop_cholesky_solve(n, n, a,b)
#endif
          else
             goto 100
          endif
...
100    acopy = a
  ...
  end subroutine coop_sympos_solve
...
100 stop "covmat_read: failed"
  ...
  subroutine coop_covmat_cholesky(this)
    class(coop_covmat)::this
    COOP_INT::info
    call coop_cholesky(this%n, this%n, this%c, info)
    if(info .ne. 0) stop "covmat_cholesky: matrix not positive definite"       
...
100 call this%normalize()    
  ...
  function coop_GelmanRubin_R(n, cov, meanscov) result(R)
    ...
    call coop_cholesky(n, n, rot, 1.d-8)
  ...
  subroutine coop_solve_constrained(m, n_known, n_unknown, C, Fmean, Ffluc)
  ...
  subroutine coop_cholesky_solve_mult(m, n, neqs, a, b)
  ...
  subroutine coop_sympos_inverse(m, n, a, epsilon)
    ...
    case default
       ...
       if(info .ne. 0) stop "sympos_inverse:matrix not positive definite"
