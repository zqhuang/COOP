...
100 stop "Error in the data file"
  ...
  subroutine get_acceleration_with_U(x, a)
    ...
    u = u - i
    d = 1.0_dl - u
    !!use U, this would be very inaccurate
    a(1) = d(2)*( &
         potential( i(1)  , i(2)  , i(3)  ) * d(3) &
         + potential( i(1)  , i(2)  , i(3)+1) * u(3) &
         - potential( i(1)+1, i(2)  , i(3)  ) * d(3) &
         - potential( i(1)+1, i(2)  , i(3)+1) * u(3) &
         ) &
         + u(2)*(  &
         potential( i(1)  , i(2)+1, i(3)  ) * d(3) &
         + potential( i(1)  , i(2)+1, i(3)+1) * u(3) &
         -potential( i(1)+1, i(2)+1, i(3)  ) * d(3) &
         -potential( i(1)+1, i(2)+1, i(3)+1) * u(3) &
         )
    a(2) = d(1)*( &
         potential( i(1), i(2), i(3)) * d(3) &
         + potential( i(1), i(2), i(3)+1) * u(3) &
    ...
    a(3) =d(1)*( &
         potential( i(1), i(2), i(3)) * d(2) &
         + potential( i(1), i(2)+1, i(3)) * u(2) &
         - potential( i(1), i(2), i(3)+1  ) * d(2) &
         - potential( i(1), i(2)+1, i(3)+1) * u(2) &
         ) &
         + u(1)*(  &
         potential( i(1)+1, i(2), i(3)  ) * d(2) &
         + potential( i(1)+1, i(2)+1, i(3)) * u(2) &
         -potential( i(1)+1, i(2), i(3)+1) * d(2) &
         -potential( i(1)+1, i(2)+1, i(3)+1) * u(2) &
         )
    a = a/dx
  ...
  function potential_interp(x)
    real(kind=DL)::x(3), potential_interp, u(3), d(3)
    integer(kind=IB)::i(3)
    u = (x - xmin)/dx+1.0_dl
    i = floor(u)
    if(any(i.le.0) .or. any(i.ge. nx))then
...
program pt
  ...
  integer(kind=IB)::i
  real(kind=DL),parameter::stepsize = 1.e-4_dl/freq
   ! call load_data("19_05_18_01.csv")
  call load_data_quick()
  y(1:3) = xini
  y(4:6) = vini
  t = 0.0_dl
  Eini  = sum(y(4:6)**2)/2.0_dl + potential_interp(y(1:3))
  write(*, "(7G14.5)") t*1.e6, y(1:3)*1.e3, y(4:6) !!output unit: microsecond, mm, m/s
  do i=1, nint(200._dl/freq/stepsize)
     call eom(n, t, y, yp)
     call RungeKutta4th(n, eom, t, y, stepsize)
     if(mod(i, 100).eq.0)   write(*, "(7G14.5)") t*1.e6, y(1:3)*1.e3, y(4:6) !!output unit: microsecond, mm, m/s
  enddo
