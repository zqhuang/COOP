...
#include "constants.h"
  type(coop_asy)::fig
  COOP_STRING::file, output, caption, xlabel, ylabel, legend, interp
  character(len = 5) tmp
  COOP_SHORT_STRING::color, linetype
  COOP_SINGLE::linewidth, legendx, legendy
  COOP_INT::i, xcol, ycol, legendcols
  logical::has_legend
  if(iargc().lt.2)then
     write(*,*) "============================================================================"
     write(*,*) "Syntax:"
  ...
  fig%color(11) = "pink"
  fig%color(12) = "yellow"
  fig%linewidth(1:3) = 1.2
  fig%linewidth(4:6) = 0.9
  fig%linewidth(7:12) = 0.6
  ...
  do i = 1, 12
     ...
     call coop_get_command_line_argument(key="xcol"//COOP_STR_OF(i), arg=xcol, default=1)
     call coop_get_command_line_argument(key="ycol"//COOP_STR_OF(i), arg=ycol, default=2)
     if(trim(legend).eq."")then
        call fig%plot_file(file, interp, xcol, ycol, color, linetype, linewidth)
     else
        call fig%plot_file(file, interp, xcol, ycol, color, linetype, linewidth, legend)
        has_legend = .true.
     endif
  enddo
  if(has_legend)then
