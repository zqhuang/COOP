...
module kessence
  ...
  implicit none
  !! ---------------- configuration ---------------------
  real*8, parameter:: rand_width = 5.d0  
  integer, parameter::dimX = 7 !expand to X^{dimX-1}, dimX>=2 
  integer, parameter::dimV = 7  !expand to phi^{dimV-1}, dimV>=1
  real*8, parameter:: khMpc_pivot = 0.05  !! k/h * Mpc
  real*8, parameter:: fit_amin = 0.75d0 !!z=0.5
  !cold dark matter + baryon; this does not include massive neutrinos
  real*8, parameter::Omega_m = 0.25
  real*8, parameter:: min_cs2 = 0.d0, max_w = -1./3., min_w = -1.e30, max_growth = 100.d0, frozen_cut  = 0.01d0    
...
contains
  ...
  function get_w() result(fail)
    ...
    integer i
    rho0_min = 0.01d0 * Omega_L
    rho0_max = 100.d0 * Omega_L
...
#include "constants.h"  
  integer i
  integer,parameter::want_samples = 200
  real*8, parameter::w0_left = -1.22, w0_right = -1.d0+0.22
  real*8 w0(want_samples), wa(want_samples)
  type(coop_asy)::w0wa
  integer ncount
  call initialize()
  call w0wa%open("wfigs/wwscatter_"//COOP_STR_OF(nint(Omega_m*100))//"_"//COOP_STR_OF(nint(rand_width))//"_"//COOP_STR_OF(dimX)//"_"//COOP_STR_OF(dimV)//".txt")
  ...
  enddo
  call w0wa%plot( x = (/ 1.d0+w0_left, 1.d0+w0_right /), y = (/ kessence_wa(w0_left, Omega_m), kessence_wa(w0_right, Omega_m) /), color="skyblue", linetype="solid", linewidth=2., legend="$\Omega_m=0."//COOP_STR_OF(nint(Omega_m*100))//"$")   
  call w0wa%dots(1.d0+w0, wa, "skyblue")
 ! call w0wa%compact_legend(0.435, 0.745)
  call w0wa%close()
end program main
