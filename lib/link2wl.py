...
replace_first("source/DataLikelihoods.f90", [r'^(\s*subroutine\s+SetDataLikelihoods\s*\(\s*Ini\s*\)\s*(\!.*)?)$', r'^(\s*call\s+BAOLikelihood\_Add\s*\(\s*DataLikelihoods\s*\,\s*Ini\s*\)\s*(!.*)?)$', r'^(\s*CosmoSettings\%use\_LSS\s*\=\s*use\_mpk\s*(\!.*)?)$'], [r'\1 !!Zhiqi added[[\n   use wl !!]]', r'\1 !!Zhiqi added[[\n   Call WLLikelihood_Add(DataLikelihoods, Ini) !!]]', r'\1 !!Zhiqi added[[\n   CosmoSettings%use_LSS = CosmoSettings%use_LSS .or. use_wl_lss !!]]'])
