...
replace_all("camb/halofit_ppf.f90", [r'^(\s*subroutine\s+NonLinear_GetNonLinRatios\s*\(\s*CAMB\_Pk\)\s*(\!.*)?)$', r'^\s*om\_m\s*\=\s*omega\_m\s*\(.*$', r'^\s*om\_v\s*\=\s*omega\_v\s*\(.*$', r'\(\s*1\.?\s*\+\s*w\_lam\s*\)'], [r'\1\n use coop_wrapper', r'  om_m = (coop_global_cdm%Omega * coop_global_cdm%rhoa4_ratio(a) + coop_global_baryon%Omega * coop_global_baryon%rhoa4_ratio(a))/coop_global_cosmology%H2a4(a)',  r' om_v = coop_global_de%Omega * coop_global_de%rhoa4_ratio(a) / coop_global_cosmology%H2a4(a) ' , r' coop_global_de%wp1 ' ])
