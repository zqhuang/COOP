...
#include "constants.h"
  ...
  logical::hot, cold
  COOP_STRING::map_file, imask_file, polmask_file, mask_file, peak_name, orient_name, output, params_file
  type(coop_stacking_options)::sto
  type(coop_healpix_maps)::hgm, mask
  COOP_REAL::threshold
  COOP_INT::i, nside_scan
  
  type(coop_general_stack_points)::points
  ...
  elseif(iargc() .eq. 1)then
     ...
     call points%init(params_file)
     write(*,*) "saving stacking points in "//trim(points%output)
     call points%write()
  else
