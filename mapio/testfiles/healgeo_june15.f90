...
200    call fp%close()
  ...
  subroutine coop_healpix_inpaint_upgrade(this, reset)
    ...
    COOP_REAL::shift(3), x(37), sumc(40), cov(40, 40), Fmean(3, 40), FFluc(3,3),s, weight(-5:5)
    logical, optional::reset
    type(coop_healpix_maps)::ltmp
    ...
    if(nside .eq. this%base_nside .and. this%first_realization)then
       ...
       call ltmp%init(nside = nside, nmaps = 1, genre = "TEMPERATURE", nested = .true.)
       ltmp%map(:,1) = this%lCT%map(:,1) + this%lMT%map(:,1)
       !!simulate the differential map
       call coop_healpix_maps_ave_udgrade(ltmp, this%sim)
       call ltmp%free()
       call this%sim%map2alm(lmax = this%sim%lmax)
       allocate(bias(0:this%sim%lmax))
       bias = 1.d0
       do l = 7, 20
          do i = -5, 5
             weight(i) = exp(-(i*10.d0/l)**2)
          enddo
          weight = weight/sum(weight)
          bias(l) = sqrt(this%filter_clL(l)/sum(this%sim%cl(l-5:l+5,1)*weight))
       enddo
       do l = 21, this%sim%lmax-5
          bias(l) = sqrt(this%filter_clL(l)/sum(this%sim%cl(l-5:l+5,1)*weight))
       enddo
       do l=this%sim%lmax-4, this%sim%lmax
          bias(l) = sqrt(this%filter_clL(l)/this%sim%cl(l, 1))
       enddo
       this%sim%alm(0:1,:,:) = 0.
       do l=2, this%sim%lmax
          this%sim%alm(l, 0, 1) =  this%filter_fluc(l)*coop_random_complex_Gaussian(.true.)  + this%sim%alm(l, 0, 1)*bias(l)*this%filter_mean(l) 
          do m = 1, l
             this%sim%alm(l, m, 1) =   this%filter_fluc(l)*coop_random_complex_Gaussian() + this%sim%alm(l, m, 1)*bias(l)*this%filter_mean(l)
          enddo
       enddo
       deallocate(bias)
       call this%sim%alm2map()
       call this%sim%convert2nested()
       this%sim%map(:,1) = (this%sim%map(:,1)*(1.-this%smooth_mask%map(:,1)**2)+this%smooth_map%map(:,1)*this%smooth_mask%map(:,1))*(1.-this%mask%map(:,1))
       return
    endif
    ...
    if(this%lM%nside .gt. 32)then
       is_boundary = .false.
    else
       call ltmp%init(nside = nside, nmaps = 1, genre = "MASK", nested = .true.)
       call coop_healpix_maps_ave_udgrade(this%smooth_mask, ltmp)
       do i = 0, npix-1
          if(all(this%lM%map(4*i:4*i+3, 1) .ge. 1.-coop_inpaint_mask_threshold))then  !!unmasked region
             is_boundary(i) = .false.
             cycle
          endif
          is_boundary(i) = (ltmp%map(i, 1) .gt. coop_inpaint_mask_threshold)
       enddo
       call ltmp%free()
    endif
    call ltmp%init(nside = this%lCT%nside, nmaps = 1, genre = "T", nested = .true.)
    call coop_healpix_maps_ave_udgrade(this%sim, ltmp)
    ltmp%map(:,1) = ltmp%map(:,1)
    do i=0, npix-1
       if(is_boundary(i))then
          ...
          this%lCT%map(4*i:4*i+3,1) = ltmp%map(4*i:4*i+3,1) !*(1.-this%smooth_mask%map(4*i:4*i+3,1)) + this%smooth_map%map(4*i:4*i+3, 1)
       endif
    enddo
    call ltmp%free()
    if(.not. any(is_boundary))return

    !!show the boundary
!!$    call ltmp%init(nside = nside, nmaps = 1, genre = "T", nested = .true.)
!!$    where (is_boundary)
!!$       ltmp%map(:,1) = real(count_boundary)
!!$    elsewhere
!!$       ltmp%map(:,1) = 0.
!!$    end where
!!$    call ltmp%write("boundary"//COOP_STR_OF(nside)//".fits")
!!$    call ltmp%free()
...
!          write(*,*) "painted "//COOP_STR_OF(npix-ib)//" boundary pixels"
          exit
       endif
       call neighbours_nest(nside, i, list, nneigh)
       nc = 4*(nneigh + 2)
