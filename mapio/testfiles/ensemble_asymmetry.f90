...
#include "constants.h"
  ...
  do i=n, 1,-1
     write(*,"(2F12.2)") sqrt(eig(i)), dot_product(diff, psi(:, i))/sqrt(eig(i))
  enddo

  print*, "chi^2(data) = ", chisq
  nlines = coop_file_numlines(fr_file) 
  ismall = 0
  ilarge = 0
  call fpsim%open(fr_file, "r")
  ...
  write(*,"(A, F13.4)") "probability of seeing chi^2 >= chi^2(data) is ", real(ilarge)/real(nlines)
