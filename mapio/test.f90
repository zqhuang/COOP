...
#include "constants.h"
  ...
  COOP_INT, parameter::lmax = lsmooth*3, nsims = 100
  ...
  bandmap = map
  window(0:1) = 0.d0
  call system("mkdir presmoothl"//COOP_STR_OF(lsmooth))
!  call map%savefig("presmoothl"//COOP_STR_OF(lsmooth)//"/data_presmoothl"//COOP_STR_OF(lsmooth)//".gif", title="data")  
  do l = 2, lmax
...
!  call lowmask%write("presmoothl"//COOP_STR_OF(lsmooth)//"/coldspot_constrained_mask_presmoothl"//COOP_STR_OF(lsmooth)//".fits")
  call lowmask%savefig("presmoothl"//COOP_STR_OF(lsmooth)//"/coldspot_constrained_mask_presmoothl"//COOP_STR_OF(lsmooth)//".gif")
!!$  stop
  invlowmask = lowmask
...
!!$  call invlowmask%savefig("invlm.gif")
  ...
  do isim = 1, nsims
     ...
     call bandmap%map2alm(lmax = lmax)
     call bandmap%alm2map()
     lowmap%map = lowmap%map+bandmap%map
     lowmap%ordering = bandmap%ordering
     if(isim .lt. 10) call bandmap%savefig("presmoothl"//COOP_STR_OF(lsmooth)//"/sim"//COOP_STR_OF(isim)//"_presmoothl"//COOP_STR_OF(lsmooth)//".gif", title="simulation #"//COOP_STR_OF(isim))
     print*, isim
...
!  call lowmap%write("presmoothl"//COOP_STR_OF(lsmooth)//"/coldspot_constrained_meanmap_presmoothl"//COOP_STR_OF(lsmooth)//".fits")
  call lowmap%savefig("presmoothl"//COOP_STR_OF(lsmooth)//"/coldspot_constrained_meanmap_presmoothl"//COOP_STR_OF(lsmooth)//".gif", title="constrained mean map")
  lowmask%map = 1.-lowmask%map
  call lowmap%apply_mask(lowmask)
  call lowmap%savefig("presmoothl"//COOP_STR_OF(lsmooth)//"/coldspot_constrained_meanmap_chopped_presmoothl"//COOP_STR_OF(lsmooth)//".gif")
