...
#include "constants.h"
  ...
  call fp%open("ffp8_noise_cls.dat","r")
  do l=0, lmax
  ...
  call fp%open("ffp8_cmb_cls.dat", "r")
  do l=0, lmax
     read(fp%unit, *) ells(l), Cls_cmb(l, :)
  enddo
  call fp%close()

  call fp%open("ffp8_cls.dat", "r")
  do l=0, lmax
     read(fp%unit, *) ells(l), Cls_total(l, :)
  enddo
  call fp%close()

  call fp%open("smica_cls.dat", "r")
  do l=0, lmax
     read(fp%unit, *) ells(l), Cls_data(l, :)
  enddo
  call fp%close()

  call fig%open("Cls_compare.txt")
...
!  call fig%curve(x = ells(2:lmax), y = Cls_noise(2:lmax, 1)*factor(2:lmax), legend = "FFP8 noise")
!  call fig%curve(x = ells(2:lmax), y = Cls_cmb(2:lmax, 1)*factor(2:lmax), legend = "FFP8 cmb")
