...
void coop_trim_C_string(char* str, char* trimmed){
  int i;
  i = 0;
  while(i<1024){
    if(str[i]<=' '){
...
void coop_fits_read_header_to_string_(char* filename, char* cards, int* nkeys){
  ...
  coop_trim_C_string(filename, trimmed);
  fits_open_file(&fptr, trimmed, READONLY, &status);
...
void coop_fits_write_image_(char * filename, int *nkeys, char *keys, char *values, int dtypes[], double * data, int * nx, int * ny) {
  ...
  naxes[1] = *ny;
  status = 0;    
  fits_create_file(&fptr, trimmed,  &status); 
