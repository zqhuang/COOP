  ...
  do ind = 0, n_sim
     if(ind.gt.ind_done)then
        ...
        if(ind .eq. 0)then
           ...
           if(trim(stack_field_name).eq."QU")then
              write(fpcheck%unit, "(4I8, 10E15.6)") patch_max%nstack_raw, sto_max%peak_pix%n, patch_min%nstack_raw, sto_min%peak_pix%n, sum(abs(patch_max%image(:,:,1))), sum(abs(patch_max%image(:,:,1))), sum(abs(patch_max%image(:,:,2))), sum(abs(patch_min%image(:,:,1))), sum(abs(patch_min%image(:,:,2))), sum(patch_max%nstack), sum(patch_min%nstack), sto_max%sigma_I, sto_min%sigma_I , imap%map(10,1), polmap%map(1024,1)                       
           else
              write(fpcheck%unit, "(4I8, 10E15.6)") patch_max%nstack_raw, sto_max%peak_pix%n, patch_min%nstack_raw, sto_min%peak_pix%n, sum(abs(patch_max%image(:,:,1))), sum(patch_max%image(:,:,1)**2), sum(abs(patch_min%image(:,:,1))), sum(patch_min%image(:,:,1)**2), sum(patch_max%nstack), sum(patch_min%nstack), sto_max%sigma_I, sto_min%sigma_I, imap%map(10,1), imap%map(1024,1)             
           endif
           call fpcheck%close()
        endif
