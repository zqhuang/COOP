...
#include "constants.h"
  ...
  do map_want = 1, nmaps
     do m_want = 0, mmax, 2
        ...
        call fig%curve(r, f(0:n,m_want/2, map_want, 0), color = "red", linetype = "solid", linewidth = 1.5, legend = "Data")
        call fig%curve(r, mean, color = "blue", linetype = "dotted", linewidth = 1.5, legend = "FFP8 mean")
        call fig%curve(r, mean+std, color = trim(coop_asy_gray_color(0.75)), linewidth = 0.25, legend = "FFP8 2-$\sigma$")
        call fig%curve(r, mean+std, color = "gray", linewidth = 0.25, legend = "FFP8 1-$\sigma$")        
