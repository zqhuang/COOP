...
#include "constants.h"
  ...
  call hp%init(nside = 2048, nmaps = 3, genre="IQU")
  call imask%init(nside = 2048, nmaps = 1, genre="MASK")
  call qmask%init(nside = 2048, nmaps = 1, genre="MASK")
  call umask%init(nside = 2048, nmaps = 1, genre="MASK")
  call imap%convert2healpix(hp, 1, imask, lower = -400., upper = 400.)
  call qmap%convert2healpix(hp, 2, qmask, lower = -250., upper = 250.)
  call umap%convert2healpix(hp, 3, umask, lower = -250., upper = 250.)
  ...
  qmask%map(:,1) = qmask%map(:,1)*umask%map(:,1)*imask%map(:,1)
