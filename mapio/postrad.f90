...
#include "constants.h"
  ...
  COOP_REAL,dimension(:,:,:),allocatable::fr, frr, fr_mean
  ...
  print*, "nmaps = ", nmaps
  print*, "nsim = ", nsims
  allocate(fr(0:n, 0:mmax/2, nmaps), fr_mean(0:n, 0:mmax/2, nmaps), frr(n_bins, 0:mmax/2, nsims), frr_mean(n_bins, 0:mmax/2), frr_cov(n_bins, n_bins, 0:mmax/2))
  frr_mean = 0.d0
  fr_mean = 0.d0
  frr_cov = 0.d0
  do isim = 1, nsims
  ...
  do m = 0, mmax/2
     ...
     do i = 0, n
        write(fp%unit, "(2E16.7)") dr*i,  sum(fr_mean(i, m, :))/nmaps
     enddo
     call fp%close()
