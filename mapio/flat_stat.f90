...
#include "constants.h"
  ...
  if(iargc().lt.1)then
     write(*,*) "Syntax:"
     write(*,*) "./FStat -map MAPFILE [-mask MASKFILE] [-map2 SECOND_MAPFILE] [-mask2 SECOND_MASKFILE] [-reg_limit REG_LIMIT(0)] [-cl_file CL_FILE]"
     stop
  endif
  if(iargc().eq.1)then
  ...
  if(trim(fname2).ne."")then
     if(trim(fmask).ne."" .or. trim(fmask2).ne."")then
        call map2%simple_stat(mean=mean2, rms=rms2, mask = mask, threshold = threshold)
        write(*,*) "correlation between two maps:", sum((map%image-mean)*(map2%image-mean2), mask = mask%image .gt. threshold)/sqrt(sum((map%image-mean)**2, mask = mask%image .gt. threshold)*sum((map2%image-mean2)**2, mask = mask%image .gt. threshold))
     else
