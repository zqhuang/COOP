...
!  logical::coop_healpix_IAU_headless_vector = .true.
  ...
  type coop_healpix_maps
   ...
   contains
     ...
     procedure :: qu2ur => coop_healpix_maps_QU2Ur
     procedure :: qu2qr => coop_healpix_maps_QU2Qr     
     procedure :: qu2QbarUbar => coop_healpix_maps_QU2QbarUbar
...
200 if(present(nmaps_to_read)) &
  ...
  subroutine coop_healpix_maps_fields_to_spins(this, imap)
    ...
    if(present(imap))then
       ...
       case("ID1", "ID2", "ZD1", "ZD2", "ED1", "ED2", "BD1", "BD2", "LNID1", "LNID2", "QD1", "QD2", "UD1", "UD2")
          this%spin(i) = 1
       case("QPOLARISATION", "QPOLARIZATION", "Q", "QT", "QLT", "QLZ", "QZ", "QLE", "QLB", "QSTOKES", "LNQ", "LNQT", "LNQLT")
          this%spin(i) = 2
          this%iq = i
       case("UPOLARISATION", "UPOLARIZATION", "U", "UT", "ULT", "ULZ", "UZ", "ULE", "ULB", "USTOKES", "LNU", "LNUT", "LNULT")
          this%spin(i) = 2
          this%iu = i
       case default
  ...
  !!transform a masked (Q, U) map to U_r map with disk size r_deg and with weight(r); the weight should be chosen such that weight(0) = 0 (avoid ambiguity)
  subroutine coop_healpix_maps_QU2Ur(this, iq, iu, iUr, mask, r_deg, weight)
    class(coop_healpix_maps)::this
    type(coop_healpix_maps)::mask
    COOP_INT::iq, iu, iUr
    COOP_INT::listpix(0:this%npix-1), nlist
    ...
    do pix=0, this%npix-1
       if(mask%map(pix, 1) > 0.)then  !calculate averaged U_r for this
          ...
          do i = 0, nlist-1
             ...
             w = w + wr
          enddo
          if(w > 0.) this%map(pix, iUr) = this%map(pix, iUr)/w
       endif
    enddo
  end subroutine coop_healpix_maps_QU2Ur


  !!transform a masked (Q, U) map to Q_r map with disk size r_deg and with weight(r); the weight should be chosen such that weight(0) = 0 (avoid ambiguity)
  subroutine coop_healpix_maps_QU2Qr(this, iq, iu, iQr, mask, r_deg, weight)
    class(coop_healpix_maps)::this
    type(coop_healpix_maps)::mask
    COOP_INT::iq, iu, iQr
    COOP_INT::listpix(0:this%npix-1), nlist
    ...
    this%map(:, iQr) = 0.d0
    do pix=0, this%npix-1
       if(mask%map(pix, 1) > 0.)then  !calculate averaged Q_r for this
          call mask%query_disc(pix, r_deg*coop_SI_degree, listpix, nlist)
          call mask%get_disc(pix, disc)
          w = 0.d0
          do i = 0, nlist-1
             ...
             w = w + wr
          enddo
          if(w > 0.) this%map(pix, iQr) = this%map(pix, iQr)/w
       endif
    enddo
  end subroutine coop_healpix_maps_QU2Qr

  !!transform a masked (Q, U) map to (Qbar, Ubar) map with disk size r_deg and with weight(r); the weight should be chosen such that weight(0) = 0 (avoid ambiguity)
  subroutine coop_healpix_maps_QU2QbarUbar(this, map, iq, iu, mask, r_deg, weight)
    class(coop_healpix_maps)::this
    ...
    do pix=0, this%npix-1
       if(mask%map(pix, 1) > 0.)then  !calculate averaged U_r for this
          ...
          enddo
          if(w > 0.)then
             map%map(pix, iq) = map%map(pix, iq)/w 
             map%map(pix, iu) = map%map(pix, iu)/w 
          endif
       endif
    enddo
  end subroutine coop_healpix_maps_QU2QbarUbar
  ...
  subroutine coop_healpix_maps_get_disc(this, pix, disc)
    ...
    disc%ny = (/  -sin(disc%phi) , cos(disc%phi) , 0.d0 /)
    call coop_vector_cross_product(disc%ny, disc%nz, disc%nx)
#else
    stop "CANNOT FIND HEALPIX"
#endif
